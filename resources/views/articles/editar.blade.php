...
@section('contenido')
    <div class="p-8 rounded-xl shadow-xl w-1/2 mx-auto bg-gray-800">
        <form name="as" method="POST" action="{{ route('articles.update', $article) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="mb-4">
                ...
                <input value="{{ @old('titulo', $article->titulo) }}"
            ...
            <div class="mb-4">
                ...
                <textarea
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="contenido" type="text" placeholder="Contenido del articulo" name="contenido" rows="8">{{ @old('contenido', $article->contenido) }}</textarea>
                @error('contenido')
                    <p class="text-red-700 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <select
                    ...
                    @foreach ($autores as $autor)
                        <option value="{{ $autor->id }}" @selected($autor->id == @old('user_id', $article->user->id))>{{ $autor->name }}</option>
                    @endforeach
                </select>
            </div>
            @error('user_id')
            ...
            <div class="flex items-center content-center mb-4">
                <img src="{{ Storage::url($article->imagen) }}" id="imagen">
            </div>
            ...
            <div class="flex mb-4">
                <div class="flex items-center mr-4">
                    <input id="publicado" type="radio" value="Publicado" name="estado" @checked(@old('estado', $article->estado) == 'Publicado')
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="publicado"
                        class="ml-2 text-sm font-bold text-green-800 dark:text-black-300">Publicado</label>
                </div>
                <div class="flex items-center mr-4">
                    <input id="borrador" type="radio" value="Borrador" name="estado" @checked(@old('estado', $article->estado) == 'Borrador')
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
